...
QStringList E2Profile::GetLastFiles()
{
	...
	for (int i = 0; i < 8; i++)
	{
		QString sp = s->value(QString("LastFile%1").arg(i), "").toString();
		if (sp.length() == 0)
		{
			break;
		}
...
void E2Profile::SetLastFiles(const QStringList &l)
{
	...
	foreach (QString t, l)
	{
		s->setValue(QString("LastFile%1").arg(i), t);
		i++;
	}
//	for (; i < 8; i++)
//	{
//		s->setValue(QString("LastFile%1").arg(i), "");
//	}
}
...
QStringList E2Profile::GetLastScripts()
{
	...
	for (int i = 0; i < 8; i++)
	{
		QString sp = s->value(QString("LastScript%1").arg(i), "").toString();
		res << sp;
	}
...
void E2Profile::SetLastScripts(const QStringList &l)
{
	...
	foreach (QString t, l)
	{
		s->setValue(QString("LastScript%1").arg(i), t);
		i++;
	}
	for (; i < 8; i++)
	{
...
void E2Profile::SetI2CBaseAddr(int base_addr)
{
	...
	{
		s->setValue("I2CBaseAddress", QString("0x%1").arg(base_addr, 0, 16));
	}
}
...
{
	s->beginGroup("SerialNumber");
	s->setValue("Address", QString("0x%1").arg(start, 4, 16, QLatin1Char('0')));		//sprintf("0x%04lX", start)
...
void E2Profile::SetCalibrationAddress(bool enabled, unsigned long start, int size, bool mtype)
{
	...
	s->setValue("StartAddress", QString("0x%1").arg(start, 4, 16, QLatin1Char('0')));	//sprintf("0x%04lX", start)
...
void E2Profile::SetCOMAddress(unsigned int com1, unsigned int com2, unsigned int com3, unsigned int com4)
{
	...
	if (com1 > 0)
	{
		str = QString("%1").arg(com1, 0, 16);

		if (com2 > 0)
		{
			str += QString(",%1").arg(com2, 0, 16);

			if (com3 > 0)
			{
				str += QString(",%1").arg(com3, 0, 16);

				if (com4 > 0)
				{
					str += QString(",%1").arg(com4, 0, 16);
				}
			}
		}
...
void E2Profile::SetLPTAddress(unsigned int lpt1, unsigned int lpt2, unsigned int lpt3)
{
	...
	if (lpt1 > 0)
	{
		str = QString("%1").arg(lpt1, 0, 16);

		if (lpt2 > 0)
		{
			str += QString(",%1").arg(lpt2, 0, 16);

			if (lpt3 > 0)
			{
				str += QString(",%1").arg(lpt3, 0, 16);
			}
		}
