...
static vMenu EepromMenu[] =
{
	...
	{"R&eset",m_Reset,isSens, notChk, noKeyLbl, noKey, noSub},
	{NULL}
};
...
static CommandObject ToolBar2[] =
{
	...
	{C_Text,txtComment,0,"","                                                                  ",CA_None,isSens,NoFrame,0,0},
...
//	sprintf(buf,STATUSBAR_PRINT, awip->GetEEPPriType(), awip->GetEEPSubType(), GetDevSize(), awip->GetCRC(), awip->IsBufChanged() ? '*' : ' ');
	...
	SetString(lblStringID, awip == 0 ? " " : awip->GetStringID());
}
...
void e2CmdWindow::Print()
{
	...
	while (k < no_line)
	{
		...
		if (curPage == 1)
		{
			...
			pdc.DrawText(0,curRow++ * cHeight, str);
			sprintf(str, "Size  : %d Bytes    CRC: %04X", GetDevSize(), awip->GetCRC());
			pdc.DrawText(0,curRow++ * cHeight, str);
		}
		curRow++;
